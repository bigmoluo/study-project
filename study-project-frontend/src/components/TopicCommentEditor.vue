...
function submitComment() {
    if (deltalToText(content.value).length > 2000) {
...
function deltaToSimpleText(delta) {
    let str = deltalToText(JSON.parse(delta))
            ...
            <div>
                <div>
                    <quill-editor style="height: 120px" v-model:content="content"
                                  placeholder="请发表友善的评论，不要用脏话骂人，多是大学生，素质高一点"/>
                </div>
                <div style="margin-top: 10px; text-align: right">
                    <div style="color: grey; font-size: 13px; display: flex">
                        当前字数 {{deltalToText(content).length}} (最大支持2000字)
                    </div>
